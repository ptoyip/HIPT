...
class HIPT_4K(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for mini_bs in range(
            ...
            features_cls256.append(
                self.model256(minibatch_256).detach().cpu()
        ...
        features_cls256 = features_cls256.to(
            self.device4k, non_blocking=True
        )  # 4. [1 x 384 x w_256 x h_256]
        features_cls4k = self.model4k.forward(
            features_cls256
    ...
    def _get_region_attention_scores(self, region, scale=1):
        ...
        attention_4k = (
            nn.functional.interpolate(
                attention_4k.unsqueeze(0), scale_factor=int(256 / scale), mode="nearest"
            )[0]
            .cpu()
            .numpy()
        )
        ...
        if scale != 1:
            batch_256 = nn.functional.interpolate(
                batch_256, scale_factor=(1 / scale), mode="nearest"
            )

        return tensorbatch2im(batch_256), attention_256, attention_4k

    def get_region_attention_heatmaps(
        self,
        x,
        offset=128,
        scale=4,
        alpha=0.5,
        cmap=cmap_map(lambda x: x / 2 + 0.5, matplotlib.cm.jet),
        threshold=None,
    ):
        r"""
        ...
        if threshold != None:
            for i in range(6):
                score256_1 = concat_scores256(
                    a256_1[:, i, :, :], w_256, h_256, size=(w_s // w_256, h_s // h_256)
                )
                score256_2 = concat_scores256(
                    a256_2[:, i, :, :], w_256, h_256, size=(w_s // w_256, h_s // h_256)
                )
                new_score256_2 = np.zeros_like(score256_2)
                new_score256_2[offset_2:w_s, offset_2:h_s] = score256_2[
                    : (w_s - offset_2), : (h_s - offset_2)
                ]
        ...
        if False:
            for j in range(6):
                ...
                Image.fromarray(region4k_hm).save(
                    os.path.join(output_dir, "%s_4k[%s].png" % (fname, j))
                )
        ...
        for j in range(6):
            ...
            new_score4k_2[offset_2:h_s, offset_2:w_s] = score4k_2[
                : (h_s - offset_2), : (w_s - offset_2)
            ]
            new_score4k_3 = np.zeros_like(score4k_3)
            new_score4k_3[offset_3:h_s, offset_3:w_s] = score4k_3[
                : (h_s - offset_3), : (w_s - offset_3)
            ]
            new_score4k_4 = np.zeros_like(score4k_4)
            new_score4k_4[offset_4:h_s, offset_4:w_s] = score4k_4[
                : (h_s - offset_4), : (w_s - offset_4)
            ]
            ...
            score4k = (
                score4k_1 + new_score4k_2 + new_score4k_3 + new_score4k_4
            ) / overlay4k
        ...
        for i in range(6):
            score256_1 = concat_scores256(
                a256_1[:, i, :, :], h_256, w_256, size=(256, 256)
            )
            score256_2 = concat_scores256(
                a256_2[:, i, :, :], h_256, w_256, size=(256, 256)
            )
            new_score256_2 = np.zeros_like(score256_2)
            new_score256_2[offset_2:h_s, offset_2:w_s] = score256_2[
                : (h_s - offset_2), : (w_s - offset_2)
            ]
            overlay256 = np.ones_like(score256_2) * 100
        ...
        for j in range(6):
            ...
            for i in range(6):
                ...
                score = (score4k * overlay4k + score256 * overlay256) / (
                    overlay4k + overlay256
                )  # factorize(score256*score4k)
                color_block = (cmap(score) * 255)[:, :, :3].astype(np.uint8)
                region4k_256_hm = cv2.addWeighted(
                    color_block,
                    alpha,
                    save_region.copy(),
                    1 - alpha,
